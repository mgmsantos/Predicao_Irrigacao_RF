...
dados_irrigacao = pd.read_csv(
    r"C:\Users\migue\Downloads\archive\irrigation_prediction.csv"
)
...
dados_irrigacao['Irrigation_Need_Class'] = (
    dados_irrigacao['Irrigation_Need']
    .map(niveis_irrigacao)
)

# Features numéricas

col_num = dados_irrigacao[[
    'Soil_Moisture',
    'Temperature_C',
    'Rainfall_mm',
    'Wind_Speed_kmh'
...
col_crop_stage_dummy = pd.get_dummies(
    dados_irrigacao['Crop_Growth_Stage'],
    prefix='Crop_Stage',
    drop_first=False
)
...
col_mulching_dummy = pd.get_dummies(
    dados_irrigacao['Mulching_Used'],
    prefix='Mulching',
    drop_first=True
)
...
dados_processado = pd.concat([
    col_num,
    col_crop_stage_dummy,
    col_mulching_dummy
], axis=1)
...
X = dados_processado.drop(
    columns=['Irrigation_Need', 'Irrigation_Need_Class']
)
...
# Como usamos sample(), vamos pegar o primeiro elemento (índice 0)
# dessa amostra para plotar a cascata
